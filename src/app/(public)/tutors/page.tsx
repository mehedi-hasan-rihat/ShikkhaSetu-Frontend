...
export default function TutorsPage() {
    ...
    const fetchTutors = useCallback(async (page = 1, category = '', search = '') => {
        setLoading(true);
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tutors?${params}`);
            if (response.ok) {
                ...
                ErrorHandler.error('Failed to load tutors');
            }
        } catch (error) {
            ErrorHandler.handleApiError(error, 'Load tutors');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            fetchTutors(1, selectedCategory, searchTerm);
        }, 500);
        
        return () => clearTimeout(timeoutId);
    }, [selectedCategory, searchTerm, fetchTutors]);

    const handlePageChange = (newPage: number) => {
        fetchTutors(newPage, selectedCategory, searchTerm);
    };

    const SkeletonCard = () => (
        <div className="bg-white rounded-2xl border-2 border-gray-200 p-4 shadow-lg animate-pulse">
            <div className="grid grid-cols-5 gap-4">
                <div className="col-span-2 flex flex-col items-center gap-3">
                    <div className="w-24 h-24 bg-gray-200 rounded-full ring-2 ring-gray-200"></div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-6">
                    <p className="text-gray-600">
                        {loading ? 'Loading...' : `${pagination.total} tutor${pagination.total !== 1 ? 's' : ''} found`}
                    </p>
                </div>
                                    ...
                                    <div className="col-span-2 flex flex-col items-center gap-3">
                                        ...
                                        <div className="flex gap-2 w-full">
                                            ...
                                            <Button
                                                ...
                                                disabled={booking === tutor.id || !tutor.availabilitySlots.length}
                                                className="flex-1 border-2 border-[#0AB5F8] text-[#0AB5F8] hover:bg-[#0AB5F8] hover:text-white px-3 py-2 text-sm"
                                            >
